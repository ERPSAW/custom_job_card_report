...
def get_data(filters):
    conditions = validate_filters(filters)
...
def validate_filters(filters):
    conditions = []
    if filters.get("name"):
        conditions.append("name LIKE '%{0}%'".format(filters.get("name")))
    if filters.get("production_item"):
        conditions.append("production_item LIKE '%{0}%'".format(filters.get("production_item")))
   
    if filters.get("custom_date"):
        conditions.append("custom_date BETWEEN '{0}' AND '{1}'".format(filters.get("custom_date")[0],filters.get("custom_date")[1]))

    if filters.get("operation"):
        conditions.append("operation LIKE '%{0}%'".format(filters.get("operation")))
